...
export function buildCodePrompt(args: {
  ...
  return [
    ...
    "禁止 localStorage、sessionStorage、IndexedDB、cookie。数据库失败时不要改走本地存储，要把错误显示出来。",
    "每个功能都要做成能操作的界面：新增、列表、删除、空状态都要有，不能只写注释或占位。",
    "每个功能的主界面元素必须带 data-feature=\"功能id\"，并且写在 index.html 里。",
    "列表没有数据时要有空状态。不要输出多个文件，不要省略内容，不要用 markdown。",
    "",
