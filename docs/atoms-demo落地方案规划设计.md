...
> **版本**：v4.3 · 2026-09-18
> **本版重点**：**去掉全部交付时间约束**；钉死「完整可用」交付基线（§0 / §4 / §12.1）；修编排图 publish、Diff 交叉引用、锚点验收措辞等自洽问题
...
| **三个界面能力** | ① 双栏工作台（左 28% 对话 / 右 72% 结果，§10.2）② **代码视图**（目录树 + 只读代码，§10.3）③ **版本卡片**（输入框上方的轮次入口） |
| **两个延展能力（本次必做）** | ① 步骤产出可编辑并重跑下游 ② 任意步骤 / 任意轮次回滚 |
| **贯穿能力（本次必做）** | 成本 / Tokens 可见 |
| **完整可用基线** | Must（M1–M11）+ 延展 1/2 + 成本可见；P2（C1–C6）不进本次验收，见 §12.1 |
| **技术栈** | Next.js 15 + TypeScript + Tailwind + shadcn/ui + Drizzle + Neon Postgres + Vercel AI SDK + DeepSeek + iframe 沙箱 |
| **交付** | 在线链接（EdgeOne Pages 主 + Cloudflare Pages 备）+ GitHub public + 说明文档 |
...
| M5 | 双栏工作台 | 左栏（窄）：对话 + 步骤卡片（Agent 角色 / 状态 / 产出摘要 / 成本），**已完成的步骤折叠为一行「已完成 N 步」可展开**；右栏（宽）：预览 / 代码的 Tab 工作区 + 工具条（见 §10.2） |
...
| C2 | **运行控制台抽屉** | 预览区底部的可展开面板：沙箱 `console` 输出 + 校验结论 + 重试记录。**数据全部已经存在**（§9.4 的 `verify_result`、§9.6 的失败原因、沙箱错误回传），只是多一个展示位。最小形态可为"仅在本轮失败时自动展开" |
| C3 | 移动端响应式适配 | 加分但不必须 |
| C4 | 分享面板带二维码 | 演示时好看 |
...
| **右栏是 Tab 工作区：预览 / 代码** | 代码可见，但**不在视觉中心**；预览始终是默认面（§2.3 决策 3 + §10.3） |
...
| 2 | 右栏顶部是 Tab 工作区 | ✅ 对齐为 **预览 / 代码** 两个 Tab | **不做"设计" Tab**：那是 Visual Editor + 主题系统，本方案不做（**§4.4 W13 / §5.2 D20**——核心原因是它会引入第二条写路径） |
...
| 6 | 预览区可展开「控制台」 | 🟡 **列为 P2**（§4.3 C2） | 价值明确，但属于新增界面件；数据已存在，不进本次完整可用验收 |
| 7 | 预览工具条在预览上方 | ✅ 对齐 | 原来放在预览下方，改为与 Tab 同高 |
| 8 | 顶层是「查看器 / 编辑器 / 设计 / 控制台」四个 Tab | ❌ 不跟 | 只保留与本方案能力对应的两个；多一个 Tab 就多一份解释成本 |
...
| 层级 | 包含 | 不包含 |
|---|---|---|
| **Must（M1–M11）** | 认证、项目、三步 Agent、审批、沙箱预览、四层持久化、增量修改（§9 全闸门）、一键发布、**全量代码视图**（目录树 + 只读高亮 + 改动标记 + 引用到对话） | — |
| **延展（S1–S3）** | 步骤产出可编辑并重跑、任意步骤/轮次回滚、成本可见 | — |
| **P2（C1–C6）** | — | 导出 zip、控制台抽屉、移动端适配、二维码分享、完整 Diff 面板、主题预设——**架构兼容，不进本次验收** |
| **明确不做（W1–W13）** | — | 见 §4.4 |
